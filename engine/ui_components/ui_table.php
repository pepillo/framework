...
class ui_table_list{
    ...
    public static function newTable($id='table'){
        $table = new ui_table_list();
        $table->id = $id;

        return $table;
    }

    public function addHeader($key='', $value='', $width=null){
        $this->header[$key]       = $value;
        $this->header_width[$key] = $width;
    }
    ...
    private function getHeader(){
        ...
        foreach($this->header as $header_key => $header_name){
            $width = '';

            if(!is_null($this->header_width[$header_key])){
                $width = "width:{$this->header_width[$header_key]}px;";
            }

            $header .= "<th style='{$width}'>{$header_name}</th>";
        }
    ...
    private function getActions($data=null){
        ...
        return "<td style='white-space: nowrap;'>{$action_html}</td>";
    }

    public function getHTML(){
        $html = "<div class='box-body'>
                    <div class='table-responsive'>
                        <table id='{$this->id}' class='table no-margin'>
                        <thead>
                            <tr>{$this->getHeader()}</tr>
                        </thead>
                            <tbody>{$this->getTableRow()}</tbody>
